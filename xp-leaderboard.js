// Per-guild XP leaderboard page logic. Kept as an external file (not inline)
// because the site CSP is `script-src 'self'` — inline scripts are blocked.
(function () {
  var cfg = (window.SITE_CONFIG || {});
  var links = cfg.links || {};
  // Wire the header CTAs from config (this page doesn't load script.js).
  ...
  function avatarOr(url, name, cls) {
    cls = cls || 'xprow-av';
    if (url) return '<img class="' + cls + '" src="' + esc(url) + '" alt="" loading="lazy" onerror="this.style.visibility=\'hidden\'">';
    var letter = esc((name || '?').charAt(0).toUpperCase());
    return '<div class="' + cls + '" style="display:flex;align-items:center;justify-content:center;font-weight:800;color:var(--text-muted)">' + letter + '</div>';
  }
  ...
  fetch('/api/dashboard/xp/leaderboard?guild=' + encodeURIComponent(guildId) + '&limit=15', { headers: { Accept: 'application/json' } })
    ...
    .then(function (data) {
      ...
      if (logo) html += '<img class="xplb-logo" src="' + esc(logo) + '" alt="" onerror="this.style.display=\'none\'">';
      ...
      podOrder.forEach(function (idx) {
        ...
        html += '<div class="pod ' + podClass[idx] + '">' +
          ...
          avatarOr(p.avatar, p.name, 'pod-av') +
      ...
      if (rest.length) {
        ...
        rest.forEach(function (p) {
          ...
          html += '<div class="xprow">' +
            ...
            avatarOr(p.avatar, p.name, 'xprow-av') +
