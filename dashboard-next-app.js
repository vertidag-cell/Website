/*
 * Arkoris — Dashboard SPA  ·  PREVIEW FORK (dashboard-next)
 * ------------------------------------------------------------
 * Verbatim fork of dashboard-app.js, loaded ONLY by the private
 * preview page dashboard-next.html. It exists so the new dashboard
 * UI can be built and tested against the real backend WITHOUT
 * touching the live dashboard-app.js. When the redesign is approved
 * we merge this back over dashboard-app.js and delete the -next
 * files. Keep it functionally identical to the live app until we
 * intentionally start changing the UI here.
 *
